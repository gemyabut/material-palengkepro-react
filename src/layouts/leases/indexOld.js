...
function LeasesPage() {
  ...
  const { profile: userProfile, setProfile, loading: profileLoading } = useProfile();
  ...
  const leaseFilter = useMemo(() => {
    ...
    if (isTenant(userRole) && userProfile?.current_tenant)
      filter.tenant = userProfile.current_tenant;
  ...
  // All hooks above; now block render until profile is loaded:
